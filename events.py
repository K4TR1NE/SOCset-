...
class VegaEvents:
    ...
    def add_default_holidays(self):
        """Добавление стандартных праздников (динамические даты)"""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        # Функция для расчета Пасхи (алгоритм Гаусса)
        def get_easter_date(year):
            ...
            if d == 29 and e == 6:
                ...
                days = 49
            if days > 31:
                return datetime(year, 4, days - 31)
            else:
                return datetime(year, 3, days)

        # Функция для расчета Масленицы (за 56 дней до Пасхи)
        def get_maslenitsa_date(year):
            easter = get_easter_date(year)
            maslenitsa = easter - timedelta(days=56)
            return maslenitsa

        holidays = [
            {'title': 'Новый год', 'description': 'Встреча Нового года! 🎄',
             'date': f'{current_year}-01-01', 'color': '#ff6b8b', 'icon': 'fa-tree'},
            {'title': 'Рождество Христово', 'description': 'Православное Рождество ✨',
             'date': f'{current_year}-01-07', 'color': '#ffb86b', 'icon': 'fa-star'},
            {'title': 'Старый Новый год', 'description': 'Старый Новый год 🎉',
             'date': f'{current_year}-01-14', 'color': '#c77dff', 'icon': 'fa-champagne-glasses'},
            {'title': 'День всех влюбленных', 'description': 'День святого Валентина 💝',
             'date': f'{current_year}-02-14', 'color': '#ff6b8b', 'icon': 'fa-heart'},
            {'title': 'День защитника Отечества', 'description': '23 февраля 🎖️',
             'date': f'{current_year}-02-23', 'color': '#8a9eff', 'icon': 'fa-shield'},
            {'title': 'Международный женский день', 'description': '8 Марта 💐',
             'date': f'{current_year}-03-08', 'color': '#ff6b8b', 'icon': 'fa-flower'},
            {'title': 'День космонавтики', 'description': '12 апреля 🚀',
             'date': f'{current_year}-04-12', 'color': '#8a9eff', 'icon': 'fa-rocket'},
            {'title': 'Праздник Весны и Труда', 'description': '1 Мая 🌸',
             'date': f'{current_year}-05-01', 'color': '#8a9eff', 'icon': 'fa-leaf'},
            {'title': 'День Победы', 'description': '9 Мая - День Великой Победы! 🎗️',
             'date': f'{current_year}-05-09', 'color': '#ff6b8b', 'icon': 'fa-ribbon'},
            {'title': 'День России', 'description': '12 июня 🇷🇺',
             'date': f'{current_year}-06-12', 'color': '#8a9eff', 'icon': 'fa-flag'},
            {'title': 'День знаний', 'description': '1 сентября 📚',
             'date': f'{current_year}-09-01', 'color': '#ffb86b', 'icon': 'fa-book'},
            {'title': 'День народного единства', 'description': '4 ноября 🤝',
             'date': f'{current_year}-11-04', 'color': '#8a9eff', 'icon': 'fa-handshake'},
            {'title': 'Хэллоуин', 'description': '31 октября 🎃',
             'date': f'{current_year}-10-31', 'color': '#ffb86b', 'icon': 'fa-ghost'},
            {'title': 'Новый год (следующий)', 'description': 'Встреча Нового года! 🎄',
             'date': f'{current_year + 1}-01-01', 'color': '#ff6b8b', 'icon': 'fa-tree'}
        ]

        # Добавляем Пасху и Масленицу (динамические даты)
        easter = get_easter_date(current_year)
        maslenitsa = get_maslenitsa_date(current_year)

        holidays.append({
            'title': 'Пасха', 'description': 'Светлое Христово Воскресение 🥚',
            'date': easter.strftime('%Y-%m-%d'), 'color': '#ffb86b', 'icon': 'fa-egg'
        })
        holidays.append({
            'title': 'Масленица', 'description': 'Проводы зимы 🥞',
            'date': maslenitsa.strftime('%Y-%m-%d'), 'color': '#ffb86b', 'icon': 'fa-sun'
        })

        for holiday in holidays:
            cursor.execute('''
                INSERT INTO events (user_id, title, description, event_date, event_type, color, icon, is_public)
                VALUES (NULL, ?, ?, ?, 'holiday', ?, ?, 1)
            ''', (holiday['title'], holiday['description'], holiday['date'],
                  holiday['color'], holiday['icon']))

        conn.commit()
        conn.close()
        print("✅ Предустановленные праздники добавлены (динамические)")
