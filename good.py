...
class VegaAuth:
    ...
    def get_posts(self, limit=20, offset=0):
        """Получить все посты с файлами из галереи"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            SELECT 
        ...
        for post in posts:
            ...
            # Получаем файлы для этого поста
            cursor.execute('SELECT * FROM gallery WHERE post_id = ?', (post_dict['id'],))
            gallery_files = cursor.fetchall()

            post_dict['files'] = [dict(f)['file_path'] for f in gallery_files] if gallery_files else []
            post_dict['author'] = {
