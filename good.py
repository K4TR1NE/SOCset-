...
class VegaAuth:
    ...
    # В класс VegaAuth добавьте эти методы:
    ...
    def send_friend_request(self, user_id, friend_id):
        ...
        if existing:
            if existing['status'] == 'accepted':
                ...
                if existing['user_id'] == user_id:
                    ...
                    cursor.execute('UPDATE users SET friends_count = friends_count + 1 WHERE id = ?', (user_id,))
                    cursor.execute('UPDATE users SET friends_count = friends_count + 1 WHERE id = ?', (friend_id,))
                    conn.commit()
                    conn.close()
        ...
        return True, f"Заявка в друзья отправлена пользователю {friend['name']}"
    ...
    def accept_friend_request(self, request_id, user_id):
        ...
        cursor.execute('UPDATE users SET friends_count = friends_count + 1 WHERE id = ?', (request['user_id'],))
        cursor.execute('UPDATE users SET friends_count = friends_count + 1 WHERE id = ?', (user_id,))
        conn.commit()
        conn.close()

        return True, "Заявка принята! Вы теперь друзья"
    ...
    def decline_friend_request(self, request_id, user_id):
        ...
        cursor.execute('DELETE FROM friends WHERE id = ? AND friend_id = ? AND status = ?',
                       (request_id, user_id, 'pending'))
    ...
    def remove_friend(self, user_id, friend_id):
        ...
        cursor.execute('UPDATE users SET friends_count = MAX(0, friends_count - 1) WHERE id IN (?, ?)',
                       (user_id, friend_id))
        conn.commit()
        conn.close()
    ...
    def get_outgoing_requests(self, user_id):
        """Получить исходящие заявки"""
        conn = self.get_connection()
        cursor = conn.cursor()

        cursor.execute('''
            SELECT f.id as request_id, u.id, u.name, u.avatar, u.zodiac, f.created_at
            FROM friends f
    ...
    def get_friend_status(self, user_id, other_id):
        ...
        if not relation:
            return 'none'  # Не друзья
        elif relation['status'] == 'accepted':
            return 'friends'  # Друзья
        elif relation['user_id'] == user_id:
            return 'sent'  # Заявка отправлена
        else:
            return 'received'  # Заявка получена
