...
def chat_page():
    ...
    # Если передан параметр user - открываем чат с этим пользователем
...
def change_password():
    ...
    # Проверяем старый пароль и меняем
...
@login_required
def get_user_likes():
    """Получить ID постов, которые лайкнул пользователь"""
    try:
        conn = sqlite3.connect('vega.db')
        cursor = conn.cursor()
