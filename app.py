...
def get_file_type(filename):
    """Определение типа файла (image/video/other)"""
...
def get_current_user():
    """Получить данные текущего пользователя (для AJAX)"""
    user = auth.get_user_by_id(session['user_id'])
    return jsonify({
        'success': True,
...
def upload_avatar():
    ...
    if not allowed_file(file.filename):
        ext = file.filename.rsplit('.', 1)[1].lower() if '.' in file.filename else 'неизвестный'
    ...
    # Удаляем старый аватар, если он загружен (не стандартный)
    user = auth.get_user_by_id(session['user_id'])
    old_avatar = user.get('avatar', '')
    if old_avatar and old_avatar.startswith('/static/uploads/avatar_'):
...
def create_post():
    ...
    for file in files:
        if file and file.filename and allowed_file(file.filename):
            ...
            file_type = 'video' if ext in ['mp4', 'webm', 'avi', 'mov', 'mkv', 'flv', 'wmv', 'm4v'] else 'image'
...
# ========== МАРШРУТЫ ДЛЯ РЕДАКТИРОВАНИЯ/УДАЛЕНИЯ ПОСТОВ ==========
...
def view_user_profile(user_id):
    ...
    # Нельзя смотреть свой профиль через этот маршрут
    ...
    # Получаем файлы из галереи для постов
    conn = sqlite3.connect('vega.db')
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM gallery ORDER BY created_at DESC')
...
@app.route('/get_mutual_friends/<int:user_id>', methods=['GET'])
@login_required
def get_mutual_friends(user_id):
    """Получить общих друзей (для AJAX)"""
    mutual = auth.get_mutual_friends(session['user_id'], user_id)
    return jsonify({'success': True, 'friends': mutual})
