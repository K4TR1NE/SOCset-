...
def send_message():
    ...
    if success:
        return jsonify({'success': True, 'message': msg, 'message_id': message_id,
                        'data': {'id': message_id, 'from_user_id': session['user_id'],
                                 'to_user_id': to_user_id, 'message': message,
...
def upload_avatar():
    ...
    if ext not in image_extensions:
        flash('Аватар должен быть изображением', 'error')
        return redirect(url_for('profile_page'))
    file.seek(0, 2)
    file_size = file.tell()
    file.seek(0)
    if file_size > 5 * 1024 * 1024:
        flash('Файл слишком большой. Максимум 5MB', 'error')
        return redirect(url_for('profile_page'))
    filename = secure_filename(f"avatar_{session['user_id']}_{datetime.now().strftime('%Y%m%d%H%M%S%f')}.{ext}")
    filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
    file.save(filepath)
    avatar_url = f'/static/uploads/{filename}'
    conn = sqlite3.connect('vega.db')
    cursor = conn.cursor()
    cursor.execute('UPDATE users SET avatar = ?, updated_at = ? WHERE id = ?',
                   (avatar_url, datetime.now().isoformat(), session['user_id']))
    conn.commit()
    conn.close()
...
@app.route('/create_repost', methods=['POST'])
@login_required
def create_repost():
    data = request.get_json() or {}
    ...
    if not post_id:
        return jsonify({'success': False, 'message': 'Пост не указан'})
    ...
    try:
        ...
        cursor.execute('SELECT COALESCE(shares, 0) FROM posts WHERE id = ?', (post_id,))
        new_count = cursor.fetchone()[0]

        conn.close()

        return jsonify({'success': True, 'message': 'Репост сделан!', 'new_count': new_count})

    except Exception as e:
        print(f"Error in create_repost: {e}")
        return jsonify({'success': False, 'message': str(e)})


@app.route('/send_repost_to_chat', methods=['POST'])
@login_required
def send_repost_to_chat():
    data = request.get_json() or {}
    post_id = data.get('post_id')
    ...
    if not post_id or not friend_id:
        return jsonify({'success': False, 'message': 'Пост или получатель не указаны'})
    ...
    try:
        ...
        clean_content = re.sub(r'<[^>]+>', '', post[1])
        clean_content = re.sub(r'\n\s*\n', '\n\n', clean_content)
        clean_content = clean_content.strip()
        if len(clean_content) > 200:
            clean_content = clean_content[:200] + '...'

        message = f"📎 Поделился(лась) постом\n\n"
        if comment:
            message += f"💬 {comment}\n\n"
        message += f"📝 {clean_content}\n\n"
        message += f"👤 Автор: {post[4]}"

        cursor.execute('''
            INSERT INTO messages (from_user_id, to_user_id, message, created_at, is_read)
            VALUES (?, ?, ?, ?, 0)
        ''', (session['user_id'], friend_id, message, datetime.now().isoformat()))
        ...
        return jsonify({'success': True, 'message': 'Репост отправлен в чат!', 'new_count': new_count})

    except Exception as e:
        print(f"Error in send_repost_to_chat: {e}")
        return jsonify({'success': False, 'message': str(e)})
...
def get_friends_list():
    ...
    try:
        ...
        cursor.execute('''
            ...
            WHERE (f.user_id = ? OR f.friend_id = ?) AND f.status = 'accepted' AND u.is_guest = 0
        ...
        for f in friends:
            result.append({
                'id': f[0],
                'name': f[1] if f[1] else 'Пользователь',
                'avatar': f[2] if f[2] else 'https://randomuser.me/api/portraits/men/1.jpg',
                'status': f[3] if f[3] else 'offline'
            })
        ...
        return jsonify({'success': False, 'friends': [], 'error': str(e)})
...
@login_required
def about_project():
    return render_template('about_project.html', current_user=auth.get_user_by_id(session['user_id']),
                           notification_count=0)
...
@login_required
def team():
    return render_template('team.html', current_user=auth.get_user_by_id(session['user_id']), notification_count=0)


@app.route('/privacy')
@login_required
def privacy():
    return render_template('privacy.html', current_user=auth.get_user_by_id(session['user_id']), notification_count=0)


@app.route('/terms')
@login_required
def terms():
    return render_template('terms.html', current_user=auth.get_user_by_id(session['user_id']), notification_count=0)


@app.route('/contacts')
@login_required
def contacts():
    return render_template('contacts.html', current_user=auth.get_user_by_id(session['user_id']), notification_count=0)
