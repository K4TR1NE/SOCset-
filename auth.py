...
class VegaAuth:
    ...
    def mark_messages_as_read(self, user_id, other_user_id):
        """Отметить сообщения от other_user_id к user_id как прочитанные"""
        conn = self.get_connection()
        cursor = conn.cursor()
    ...
    def get_posts(self, limit=20, offset=0):
        """Получить все посты для ленты (только публичные, не репосты)"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            SELECT 
            ...
            AND (posts.is_repost = 0 OR posts.is_repost IS NULL)
    ...
    def get_user_posts(self, user_id, limit=50):
        """Получить все посты пользователя для профиля (включая репосты)"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            SELECT 
        ...
        for post in posts:
            ...
            if post_dict.get('original_author_id') and post_dict['original_author_id'] is not None:
                post_dict['is_repost'] = True
                post_dict['original_author'] = {
                    'id': post_dict['original_author_id'],
